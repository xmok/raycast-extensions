...
export default function ViewLabels({ project }: { project: Project }) {
  ...
  return (
    <List isLoading={isLoading} searchBarPlaceholder="Search label" pagination={pagination}>
      {labels.map((label) => (
        <List.Item
          key={label.id}
          icon={{ source: Icon.CircleFilled, tintColor: label.color }}
          title={label.name}
          accessories={[{ date: new Date(label.created_at) }]}
          actions={
            <ActionPanel>
              <Action.Push
                icon={Icon.Pencil}
                title="Update Label"
                target={<UpdateLabel project={project} initialLabel={label} onUpdated={revalidate} />}
              />
              <Action.Push
                icon={Icon.Plus}
                title="Add Label"
                target={<AddLabel project={project} onAdded={revalidate} />}
              />
            </ActionPanel>
          }
        />
