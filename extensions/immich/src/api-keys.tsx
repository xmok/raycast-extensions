import { ApiKeyCreateDto, ApiKeyResponseDto, createApiKey, deleteApiKey, getApiKeys, isHttpError, Permission } from "@immich/sdk";
...
function NewAPIKey() {
    ...
    const { itemProps, handleSubmit, values } = useForm<FormValues>({
        async onSubmit(values) {
            ...
            try {
                const res = await createApiKey({apiKeyCreateDto: {name: values.name, permissions: values.selectAll ? Object.values(Permission) : values.permissions as Permission[]}})
                toast.style = Toast.Style.Success;
                toast.title = "Created"
                ...
                toast.style = Toast.Style.Failure;
                toast.title = "Failed"
                toast.message = isHttpError(error) ? `${Array.isArray(error.data.message) ? error.data.message[0] : error.data.message}` : `${error}`;
            }
        },
        validation: {
    ...
    return <Form actions={<ActionPanel>
        ...
        {!values.selectAll && <Form.TagPicker title="Permission" placeholder="Search" {...itemProps.permissions}>
            {Object.entries(Permission).map(([key, val]) => <Form.TagPicker.Item key={key} title={key} value={val} />)}
        </Form.TagPicker>}
    </Form>
}