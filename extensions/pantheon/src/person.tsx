...
export default function Person() {
    ...
    return <List isLoading={isLoading} isShowingDetail={isShowingDetail} throttle pagination={pagination} searchBarPlaceholder={`Search by ${query.key}`} onSearchTextChange={(val) => setQuery(prev => ({...prev, val}))} searchBarAccessory={<List.Dropdown tooltip="Search">
        ...
        {persons.map(person => <List.Item key={person.id} title={person.name} icon={{ source: person.gender==="M" ? Icon.Male : Icon.Female, tintColor: person.alive ? Color.Green : Color.Red }} subtitle={!isShowingDetail ? person.slug : undefined} accessories={isShowingDetail ? undefined : [{tag: person.occupation}]} detail={<List.Item.Detail markdown={person.description || "<NO DESCRIPTION>"} metadata={<List.Item.Detail.Metadata>
            ...
            <List.Item.Detail.Metadata.Label title="Gender" text={person.gender} icon={person.gender==="M" ? Icon.Male : Icon.Female} />
            <List.Item.Detail.Metadata.Label title="Alive" icon={person.alive ? Icon.Check : Icon.Minus} />
            {person.twitter ? <List.Item.Detail.Metadata.Link title="Twitter" text={person.twitter} target={`https://twitter.com/${person.twitter}`} /> : <List.Item.Detail.Metadata.Label title="Twitter" icon={Icon.Minus} />}
