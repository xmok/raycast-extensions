...
{!isLoading && !posts.length ? <List.EmptyView title="Start by creating a post" description="Here you can keep track of all the posts made to your feedback board." actions={<ActionPanel>
<Action.Push icon={Icon.PlusCircle} title="Create First Post" target={<NewPost />} onPop={mutate} />
</ActionPanel>} /> : posts.map(post=> <List.Item key={post.id} title={post.title} subtitle={new Date(post.date).toDateString()} accessories={[post.commentCount ? {icon:Icon.SpeechBubble, text: post.commentCount.toString()} : {}, {tag: post.postCategory.category}, {tag: {value: post.postStatus.name, color: post.postStatus.color}}]} actions={<ActionPanel>
<Action.Push icon={Icon.SpeechBubble} title="View Comments" target={<ViewComments post={post} />}  onPop={mutate}/>
    <Action.OpenInBrowser url={buildFeaturebaseUrl(`p/${post.slug}`)} />
    <Action.Push icon={Icon.PlusCircle} title="Create Post" target={<NewPost />} onPop={mutate} shortcut={Keyboard.Shortcut.Common.New} />
<Action icon={Icon.Trash} title="Delete Post" onAction={() => confirmAlert({
    ...
    primaryAction: {
        ...
        async onAction() {
            ...
            try {
                await mutate(
                    featurebase.posts.delete({id: post.id}), {
...
function NewPost() {
    ...
    const {isLoading, data: boards} = useCachedPromise(async() => {
...
<Form.Dropdown title="Board" {...itemProps.category}>
    {boards.map(board => <Form.Dropdown.Item key={board.id} title={board.category} value={board.category} />)}
</Form.Dropdown>
...
function ViewComments({post}:{post:Post}) {
    const {isLoading, data: comments, pagination,mutate}  = useCachedPromise((submissionId: string) => async(options) => {
...
return <List isLoading={isLoading} pagination={pagination} isShowingDetail>
    {comments.map(comment => <List.Item key={comment.id} icon={{ source:comment.authorPicture, fallback: Icon.SpeechBubble}} title={comment.author} detail={<List.Item.Detail markdown={turndownService.turndown( comment.content)} metadata={<List.Item.Detail.Metadata>
        <List.Item.Detail.Metadata.Label title="Upvotes" icon={Icon.ChevronUp} text={comment.upvotes.toString()} />
    ...
    primaryAction: {
        ...
        async onAction() {
            ...
            try {
                await mutate(
                    featurebase.comment.delete({id: comment.id}), {
                        optimisticUpdate(data) {
        ...
        <Action.Push icon={Icon.Plus} title="New Comment" target={<NewComment postId={post.id} />} onPop={mutate} />
    </ActionPanel>} />)}
</List>
}
