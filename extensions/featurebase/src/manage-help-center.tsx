...
export default function ManageHelpCenter() {
    ...
    const {isLoading, data: articles, pagination,mutate}  = useCachedPromise((state) => async(options) => {
    ...
    return <List isLoading={isLoading} pagination={pagination} searchBarAccessory={<List.Dropdown tooltip="State" onChange={setState} storeValue>
...
{articles.map(article=> <List.Item key={article.articleId} icon={Icon.Book} title={article.title} accessories={[{tag: article.state===ArticleState.Draft ? "Draft" : "Published"}]} actions={<ActionPanel>
    <Action.OpenInBrowser url={`https://${article.organizationId}.featurebase.app/dashboard/articles/${article.articleId}`} />
    <Action.Push icon={Icon.Plus} title="New Article" target={<NewArticle />} onPop={mutate} />
<Action icon={Icon.Trash} title="Delete Article" onAction={() => confirmAlert({
    ...
    primaryAction: {
        ...
        async onAction() {
            ...
            try {
                await mutate(
                    featurebase.helpCenter.articles.delete({id: article.articleId}), {
